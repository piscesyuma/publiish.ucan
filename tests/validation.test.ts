...
test('should fail with non base64url header', async () => {
  try {
    ...
    assert.instance(error, Error)
    assert.match((error as Error).message, `Can't parse: test*: Can't parse as base64url.`)
  }
})
...
test('should fail with non base64url signature', async () => {
  ...
  try {
    ...
    assert.match((error as Error).message, `Non-base64url character`)
  }
})
...
test('should fail with bad signature', async () => {
  ...
  try {
    ...
    assert.instance(error, Error)
    assert.match((error as Error).message, `Invalid UCAN: ${jwt}: Signature invalid.`)
  }
})
...
test('should fail with expired jwt', async () => {
  ...
  try {
    ...
    assert.instance(error, Error)
    assert.match((error as Error).message, `Invalid UCAN: ${jwt}: Expired.`)
  }
})
